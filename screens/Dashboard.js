...
import { getTransactions, deleteTransaction, updateTransaction } from "../storage/transactionStorage";
...
export default function Dashboard() {
  ...
  const handleSaveEdit = async () => {
    ...
    if (!Number.isFinite(amt)) { Alert.alert('Enter a valid amount'); return; }
    const updated = {
      id: selected.id,
      amount: amt,
    ...
    <Screen>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          <Text style={[styles.welcomeText, { color: theme.colors.text }]}>Dashboard</Text>
        </View>
        <TouchableOpacity>
      ...
      <View style={[styles.balanceCard, { backgroundColor: theme.colors.card }]}>
        ...
        <View style={styles.balanceRow}>
          ...
          <TouchableOpacity onPress={() => setShowBalance(!showBalance)}>
            <Feather name={showBalance ? "eye" : "eye-off"} size={24} color={theme.colors.muted} />
          </TouchableOpacity>
        </View>
        <Text style={[styles.accountNumber, { color: theme.colors.muted }]}>1289440585</Text>
      </View>
          ...
          >
            <View style={styles.transactionLeft}>
              <View style={[
                ...
                { backgroundColor: item.type === 'income' ? (theme.colors.accent + '20') : theme.colors.dangerLight }
              ]}>
                <Feather
                  name={item.type === 'income' ? 'arrow-down' : 'arrow-up'}
                  size={20}
                  color={item.type === 'income' ? theme.colors.accent : theme.colors.danger}
                />
              </View>
              <View>
            ...
            <Text style={[styles.transactionAmount, { color: item.type === 'income' ? theme.colors.accent : theme.colors.danger }]}> 
          ...
          <TouchableWithoutFeedback>
            <View style={styles.modalCard}>
              ...
              <View style={styles.modalBody}>
                <Text style={styles.modalDesc}>{selected ? selected.description : ''}</Text>
                <Text style={styles.modalAmount}>{selected ? bdtFormatter.format(selected.amount) : ''}</Text>

                <TouchableOpacity style={[styles.modalButton, styles.editButton]} onPress={() => {
                  // Navigate to AddTransaction tab in edit mode with the transaction
                  closeModal();
                  navigation.navigate('Wallet', { transaction: selected, edit: true });
                }}>
                  <Text style={styles.modalButtonText}>Edit</Text>
                </TouchableOpacity>

                <TouchableOpacity style={[styles.modalButton, styles.deleteButton]} onPress={async () => {
                  // Confirm then delete
                  if (!selected) return;
                  Alert.alert('Delete', 'Are you sure you want to delete this transaction?', [
                    { text: 'Cancel', style: 'cancel' },
