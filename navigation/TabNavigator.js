...
const getIconComponent = (icon, focused, theme) => {
  ...
  switch (icon) {
    ...
    case 'credit-card':
      return focused ?
        <MaterialIcons name="account-balance-wallet" size={22} color={activeColor} /> :
    ...
    default:
      return <Feather name={icon} size={22} color={focused ? activeColor : inactiveColor} />;
  }
};

const CustomTabBarIcon = ({ focused, icon, theme }) => (
  <View style={[
    styles.iconContainer,
    focused && styles.activeIconContainer
  ]}>
    <View style={[focused ? styles.filledIconWrapper : null, { backgroundColor: focused ? (theme?.colors?.background || '#F5F7FF') : 'transparent' }]}>
  ...
  return (
    ...
    >
      ...
      <Tab.Screen 
        ...
        options={{
          tabBarIcon: ({ focused }) => (
            <CustomTabBarIcon focused={focused} icon="credit-card" theme={theme} />
          ),
        }}
      />
      <Tab.Screen 
        ...
        options={{
          tabBarIcon: ({ focused }) => (
            <CustomTabBarIcon focused={focused} icon="bar-chart-2" theme={theme} />
          ),
        }}
      />
      <Tab.Screen 
